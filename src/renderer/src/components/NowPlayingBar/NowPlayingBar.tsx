...
function Tag({ label, variant = "default" }: TagProps) {
	return (
		<span
			className={`${styles.tag} ${variant === "primary" ? styles.tagPrimary : ""}`}
		>
			{label}
		</span>
	);
...
export default function NowPlayingBar({ track, config }: NowPlayingBarProps) {
	...
	const tags = useMemo(() => {
		...
		if (track.bpm)
			result.push({ label: `${Math.round(track.bpm)}BPM`, variant: "default" });
		if (track.initialKey)
			result.push({
		...
		if (track.year)
			result.push({ label: String(track.year), variant: "default" });
	...
	return (
		<div className={styles.nowPlayingBar}>
			...
			<div className={styles.waveformSection}>
				<button type="button" className={styles.playIndicator}>
