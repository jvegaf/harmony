...
export const formatOpenKey = (key: string | null | undefined): string => {
  ...
  let standard = normalized.replace(/minor/i, 'm').replace(/min/i, 'm').replace(/major/i, '').replace(/maj/i, '');
...
/**
 * Returns a sortable value for an OpenKey.
 * OpenKey consists of a number (1-12) and a mode ('m' or 'd').
 * To group by number and then mode, we create a composite score.
 * Example: '1m' -> 10, '1d' -> 11, '12m' -> 120, '12d' -> 121
 */
const getOpenKeySortValue = (openKey: string | null | undefined): number => {
  if (!openKey) return 999; // Put tracks without key at the end
  const formatted = formatOpenKey(openKey);
  const match = formatted.match(/^([1-9]|1[0-2])([md])$/i);
  if (!match) return 999;

  const num = parseInt(match[1], 10);
  const mode = match[2].toLowerCase() === 'm' ? 0 : 1;
  return num * 10 + mode;
};

/**
 * Automatically sorts tracks based on the priority setting.
 * @param tracks The list of tracks to sort
 * @param priority 'harmony' sorts by Camelot/OpenKey first, then rating. 'energy' sorts by Rating first, then Camelot/OpenKey.
 * @returns A new array of sorted tracks
 */
export const autoOrderTracks = (tracks: Track[], priority: 'harmony' | 'energy'): Track[] => {
  return [...tracks].sort((a, b) => {
    const keyA = getOpenKeySortValue(a.initialKey);
    const keyB = getOpenKeySortValue(b.initialKey);
