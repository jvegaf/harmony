 ...
 */
export const selectInStockProducts = (state: ComputedStore) => state.products.filter(product => product.inStock);

/**
 * Selector: Count items in cart
 */
export const selectCartItemCount = (state: ComputedStore) => state.cart.reduce((sum, item) => sum + item.quantity, 0);
