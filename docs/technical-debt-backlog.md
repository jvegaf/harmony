...
**Problema Identificado**: La función `setLibrarySourceRoot()` ejecutaba múltiples IPC round-trips (scan → import → filter → insert × N → config), con orquestación compleja en el renderer que bloqueaba la UI durante imports largos.
...
**Análisis Realizado**: Tras investigación exhaustiva del codebase, se determinó que `highlightPlayingTrack` era **código muerto (dead code)**:
...
**Implementación Actual (Correcta)**: El highlight de track en reproducción se gestiona eficientemente en `TrackList.tsx`:
...
**Solución Implementada**: ✅ Removidas 4 referencias de código muerto:
...
**Conclusión del Análisis**: Después de investigar el código, se determinó que **no existe un estado global `selectedTracks` en `useLibraryStore`**. La selección de tracks es manejada localmente por AG Grid dentro del componente `TrackList.tsx`.
...
**Problema Identificado**: El drag-and-drop en playlists solo permitía mover un track a la vez, incluso cuando había múltiples tracks seleccionados. Si el usuario tenía 10 tracks seleccionados, debía arrastrarlos uno por uno.

**Hallazgo Importante**: El backend (`database.ts#reorderTracks`) ya estaba diseñado para manejar múltiples tracks desde el principio. El problema estaba en el frontend que solo pasaba `[draggedTrack]` en lugar de todos los tracks seleccionados.
...
**Archivo**: `src/renderer/src/stores/PlaylistsAPI.ts:189` _(DOCUMENTADO)_  
...
**Contexto**: El TODO sugería que los M3U exportados usaban paths relativos cuando deberían ser absolutos.
...
**Contexto**: El hook `useCurrentViewTracks()` asume que siempre hay tracks para mostrar (library o playlist), pero falla silenciosamente en la página de Settings.

**Análisis**: Probablemente no es necesario soportar Settings aquí porque:
...
**Archivo**: `src/preload/types/beatport/compat.ts:7` _(ELIMINADO)_  
**Tipo**: Code cleanup  
**TODO Original**:
...
**Contexto**: Este archivo proporcionaba tipos legacy (`BeatportCandidate`) para compatibilidad con código viejo que aún no se ha migrado a `TrackCandidate`.
